...
public class Raycast : MonoBehaviour
{
    ...
    void Drag() {
        ...
        float distance ;
            if (plane.Raycast(ray, out distance)) {
                grabbed.rigidbody.position = ray.origin + ray.direction * grabDistance;
    ...
    void Curve()
    {
        Vector3 size = grabbed.collider.bounds.size;
        ...
        //Debug.Log(direction);
        float radius = 2f * Mathf.PI * (size.y) * 3f;
